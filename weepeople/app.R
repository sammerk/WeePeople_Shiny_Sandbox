...
library(sysfonts)
font_add("weepeople", "www/weepeople.ttf")
...
server <- function(input, output, session) {
    ...
    #define factor levels for later usage
    named_letters <- c(letters, LETTERS)
    names(named_letters) <- c(letters, LETTERS)
    # Reactive data storage for points
    points <- reactiveVal(
        data.frame(x = character(0), 
                   y = numeric(0), 
                   icon = factor(character(0), levels = c(letters, LETTERS))))
    
    # Render the ggplot
    output$plot <- renderPlot({
        # Create the base plot
        ggplot(points(), aes(x = x, y = y, group = y, shape = icon)) + 
